...
#--------------------------------------------------------------------------------------------------------------------
    ...
    def get_by_id(cls, data):
        result = []
        query = "SELECT id FROM users_db WHERE first_name = %(fname)s AND last_name = %(lname)s"
    ...
    @classmethod
    def update(cls, data):
        query = "UPDATE users_db SET first_name = %(fname)s, last_name = %(lname)s, email = %(email)s, updated_at = NOW() WHERE id= %(id)s;"
        return connectToMySQL("user_db").query_db(query, data)
